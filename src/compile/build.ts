...
function spawnProcess(step: Step): ProcessEnv {
    ...
    if (!step.isExternal &&
        ...
        if (args && !step.name.endsWith(lw.constant.MAGIC_PROGRAM_ARGS_SUFFIX)) {
            ...
            lw.compile.process = lw.spawnProc(`${step.command} ${args[0]}`, [], {cwd: path.dirname(step.rootFile), env, shell: true})
        } else {
            lw.compile.process = lw.spawnProc(step.command, args, {cwd: path.dirname(step.rootFile), env})
        }
        ...
        logger.log(`cwd: ${cwd}`)
        lw.compile.process = lw.spawnProc(step.command, step.args, {cwd, env})
    } else {
        logger.log(`cwd: ${step.cwd}`)
        lw.compile.process = lw.spawnProc(step.command, step.args, {cwd: step.cwd})
    }
